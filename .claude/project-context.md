...
2. **フェーズ2: 修正提案**
   - 具体的な修正内容を記述（変更ファイル、内容、期待結果）
   - リスク評価（影響機能、ロールバック方法、テスト方法）
   - 代替案の提示（可能な場合）
...
**リスク別ガイドライン**:
- **🟢 低リスク**: UIテキスト修正、ログ追加、タイポ修正（承認不要）
- **🟡 中リスク**: 新機能追加、設定変更、API追加（提案・承認必須）
- **🔴 高リスク**: DBスキーマ変更、RLSポリシー変更、セキュリティ変更（提案・承認 + 詳細レビュー）
