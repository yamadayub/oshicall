...
- **判定方法**: ハイブリッド方式（優先順位順）
  1. **Daily.co Webhookイベントログが存在する場合**: 厳密な判定を実施
     - インフルエンサー参加必須（`participant.joined`イベント）
     - 規定時間経過による自動終了必須（`room.ended`イベント、`room_end_reason === 'duration'`）
     - 途中退出なし必須（開始時刻から終了時刻まで連続参加、`participant.left`イベントで確認）
  2. **Daily.co Webhookイベントログが存在しない場合**: `purchased_slots`テーブルの情報で判定
     - インフルエンサー参加必須（`influencer_joined_at`が存在）
     - 開始時刻前に参加（`influencer_joined_at <= scheduled_start_time`）
     - 予定終了時刻まで留まっている（`call_ended_at >= scheduled_end_time`）
     - 途中退室の判定は緩和（`call_actual_duration_minutes >= duration_minutes`で概算判定）
- **結果**: Webhookの有無に関わらず、公平な決済判定が実現される

### BR-005: 決済タイミング
- **説明**: Talk完了後の決済処理に関するルール
- **実行タイミング**: 以下のいずれかのタイミングで決済処理を実行
  1. **Daily.co Webhook受信時**: `room.ended`イベント受信時に自動実行（推奨）
  2. **手動終了時**: `/api/calls/end-call`エンドポイント呼び出し時に実行（Webhook未受信時のフォールバック）
- **判定結果による処理**:
  - Talk正常完了: 決済確定（Capture）
  - Talk不成立: 与信解放（Cancel）
  - インフルエンサーno-show: 与信解放（Cancel）
- **結果**: Webhookの有無に関わらず、ファンは正常完了時のみ課金され、不公平な決済を回避できる
...
6. 規定時間経過で自動終了、または手動終了
   ↓
7. 決済処理実行（以下のいずれか）
   a. Daily.co Webhook受信時（`room.ended`イベント）→ 自動実行
   b. 手動終了時（`/api/calls/end-call`呼び出し）→ 自動実行
   ↓
8. Talk完了判定（ハイブリッド方式）
   - Webhookイベントログがある場合: 厳密な判定
   - Webhookイベントログがない場合: `purchased_slots`情報で判定
   ↓
9. 正常完了の場合: 決済確定
   不成立の場合: 与信解放
   ↓
10. call_slots.status が 'completed' に更新
```
...
| 2026-01-03 | BR-004, BR-005を更新 | 決済判定をハイブリッド方式に変更、手動終了時も決済処理を実行するように変更 | - |
