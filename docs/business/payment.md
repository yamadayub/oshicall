...
- **送金タイミング**: 
  ...
  - 決済確定後、Stripe Webhook（`payment_intent.succeeded`）を受信した時点で送金（Transfer）を実行
- **理由**: 
  - プラットフォームアカウントへの入金が確認されてから送金することで、残高不足エラーを回避
  - ファンからの入金とインフルエンサーへの送金を分離することで、コスト負担を適切に管理
- **結果**: ファンからの入金確認後に送金されるため、確実な送金が実現される
